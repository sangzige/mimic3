...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
        ...
        # Input layers and masking。这里shape其实定义的是每个样本（病人）的格式，没有包含bs表明输入是序列。即每一个样本的维度是timesteps，input_dim
        X = Input(shape=(None, input_dim), name='X')
        inputs = [X]
        # Masking 层的作用是忽略某些时间步的值（通常是填充部分），在训练时不会计算这些值的梯度。
        mX = Masking()(X)


        # 深度监督和双向LSTM二选一，这里默认是使用双向LSTM
        if deep_supervision:
        ...
        for i in range(depth - 1):
            ...
            lstm = LSTM(units=num_units,
                        ...
                        dropout=dropout)
            # num_units 代表 LSTM 层的隐藏状态的维度，也就是每个时间步的输出向量的大小
            # return_sequences 这个参数表示在每个时间步都会返回 LSTM 层的输出，而不仅仅是最后一个时间步的输出
        ...
        # Output module of the network target_repl和deep_supervision默认是0，因此第二个LSTM单元只返回最终时间步的结果
        ...
        if target_repl:
            ...
            # 最终走这里，并且final_activation取决于任务类型，一开始有定义
            #num_classes默认是1，但是住院时间预测可能是10分类，因此就为10
            # sigmiod输出的是正例的结果，而不需要两种都输出
